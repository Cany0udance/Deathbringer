...
public class ShadowUtility {
    ...
    public static void triggerGeneralShadowEffect(AbstractCard playedCard) {
        ...
        for (AbstractCard c : p.hand.group) {
            if (c.tags.contains(deathbringer.character.Deathbringer.Enums.SHADOW) && c != playedCard) {
                shadowCards.add(c);
                if (c.tags.contains(deathbringer.character.Deathbringer.Enums.UMBRA)) {
                    umbraCards.add(c);
                } else if (c.tags.contains(deathbringer.character.Deathbringer.Enums.PENUMBRA)) {
                    penumbraCards.add(c);
                }
            }
        }
...
// Assuming 'playedCard' is the card initially played and 'randomShadowCard' is the "Lights Out" card
                if (randomShadowCard.cardID.equals(LightsOut.ID)) {
                    ...
                    if (randomShadowCard.tags.contains(deathbringer.character.Deathbringer.Enums.SHADOWPLAY)) {
                ...
                if (finalRandomShadowCard.tags.contains(deathbringer.character.Deathbringer.Enums.PENUMBRA)) {
            ...
            if (hasShattered && !shatteredTriggered || hasRepaired) {
                ...
                if (m != null && !m.isDying && !m.isDead) {
                    ...
                    if (randomShadowCard.tags.contains(deathbringer.character.Deathbringer.Enums.PENUMBRA)) {
                        if (hasCloth && !clothTriggered) {
                            com.megacrit.cardcrawl.relics.AbstractRelic tornCloth = p.getRelic(TornCloth.ID);
                            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new IntangiblePlayerPower(p, 1), 1));
                            tornCloth.flash();  // Flash the relic
    ...
    private static void triggerLightsOutEffect(AbstractPlayer p, AbstractCard excludeCard1, AbstractCard excludeCard2) {
        for (AbstractCard c : new ArrayList<>(p.hand.group)) { // Use a copy of the group to avoid ConcurrentModificationException
            if (c.tags.contains(deathbringer.character.Deathbringer.Enums.SHADOW) && c != excludeCard1 && c != excludeCard2) { // Exclude both specified cards
                ...
                if (cardToDiscard.tags.contains(deathbringer.character.Deathbringer.Enums.PENUMBRA)) {
                    boolean hasCloth = p.hasRelic(TornCloth.ID);
                    if (hasCloth && !clothTriggered) {
                        com.megacrit.cardcrawl.relics.AbstractRelic tornCloth = p.getRelic(TornCloth.ID);
                        AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new IntangiblePlayerPower(p, 1), 1));
